...
static void read_msrs(const void *buf){
	int rc;
	assert( msr0_fd > 0 );
	//rc = pread( msr0_fd, &my_msrs.mperf, sizeof(my_msrs.mperf), IA32_MPERF );	assert( 8 == rc );
	
	//Value skler and jorge are changing, rest is writted by Dr. Roundree
	rc = pwrite( msr0_fd, &buf, sizeof(my_msrs.mperf), IA32_MPERF );	assert( 8 == rc );
...
static void dump_msrs(){
	...
	if( !initialized ){
		fprintf(stdout, "RRR aperf mperf\n");
		initialized = 1;
	}
